	...
	const verifyUser = async () => {
		...
		try {
			const res = await fetch(
				`${confs.urlDatos}:${confs.portDatos}/users?email=${profile.email}`
			);
		...
		<div>
			...
			<div className="row ">
				<div className="col-sm-auto">
					{canDownload === 0 && <div>cargando...</div>}
					{canDownload === 1 && (
						<button
							...
							title="Inicie Sesión para Descargar"
						>
							<i class="fas fa-cloud-download-alt"></i> Descargar
						</button>
					)}
					{canDownload === 2 && <Formulario refresh={verifyUser} />}
					{canDownload === 3 && (
						<button
							className="btn btn-primary btn-sm"
